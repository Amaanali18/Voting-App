...
const Vote = () => {
    ...
    useEffect(() => {
        if (!pollName) return
        api.get(`/api/voting/${encodeURIComponent(pollName)}`)
            .then(({ data }) => {
    ...
    const handleVote = async () => {
        ...
        try {
            await api.post(`/api/voting/${encodeURIComponent(pollName)}/vote`, { optionIndex: selected })
            setSubmitted(true)
            toast.success('Vote submitted!')
            const updated = await api.get(`/api/voting/${encodeURIComponent(pollName)}`)
